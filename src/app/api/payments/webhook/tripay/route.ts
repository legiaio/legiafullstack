...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify webhook with Tripay
    const webhookData = await paymentService.verifyWebhook('tripay', payload, signature || undefined);
    ...
    if (webhookData.status === 'paid' && payment.order.status !== 'PAID') {
      ...
      console.log(`Order ${payment.orderId} has been paid via Tripay`);
    }
    ...
    try {
      ...
      await prisma.webhookLog.create({
        data: {
          gateway: 'TRIPAY',
          paymentId: payload.reference || 'unknown',
          orderId: payload.merchant_ref || 'unknown',
          status: 'error',
